...
def preprocess_conversation(conversation, tokenizer, exclude_system_prompt=False, retry_on_failure=False):
    ...
    template_tokens = tokenize_conversation(
        conversation, tokenizer, role_overrides={"gpt": SENTINEL_TOKEN}, exclude_system_prompt=exclude_system_prompt
    )
    ...
    if i < len(template_tokens):
        if not retry_on_failure:
            return preprocess_conversation(
                conversation, tokenizer, exclude_system_prompt=exclude_system_prompt, retry_on_failure=True
            )
