...
def tokenize_conversation(messages, tokenizer, include_generation_prompt=False, role_overrides=None, exclude_system_prompt=False):
    ...
    formatted_prompt = tokenizer.apply_chat_template(normalized_conversation, add_generation_prompt=include_generation_prompt, tokenize=False)
...
def preprocess_conversation(conversation, tokenizer, exclude_system_prompt=False, retry_on_failure=False):

    input_tokens = tokenize_conversation(conversation, tokenizer, exclude_system_prompt=exclude_system_prompt)
    ...
    template_tokens = tokenize_conversation(conversation, tokenizer, role_overrides={"gpt": SENTINEL_TOKEN}, exclude_system_prompt=exclude_system_prompt)
    ...
    if i < len(template_tokens):
        if not retry_on_failure:
            return preprocess_conversation(conversation, tokenizer, exclude_system_prompt=exclude_system_prompt, retry_on_failure=True)
